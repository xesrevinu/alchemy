 ...
 * Railway Service; the only cloud touch is the state store.
 *
 * Coverage:
...
const MARKER = "Hello from vinext on Railway!";
const MARKER_V2 = "Hello from vinext on Railway! [dev-v2]";
...
test(
  ...
  async () => {
    ...
    expect(home).toContain("vinext on Railway");
